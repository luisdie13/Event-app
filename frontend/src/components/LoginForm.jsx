...
// Usamos el ID de rol que confirmaste en DataGrip
...
const LoginForm = () => {
  ...
  // Determina a dónde ir después del login (generalmente la página previa)
  // Pero si viene de /register, forzamos a /
  const from = location.state?.from?.pathname || "/";
  const targetPath = from === "/register" ? "/" : from;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!response.ok) {
        setIsError(true);
        setMessage(data.message || 'Credenciales incorrectas. Verifica tu email y contraseña.');
        return;
      } 
      ...
      setTimeout(() => {
          if (data.user.role_id === ADMIN_ROLE_ID) {
              // Si es administrador, redirigir al dashboard de admin
              navigate('/admin/dashboard', { replace: true });
          } else {
              // Si es usuario normal o miembro, redirigir a la página de inicio o a la página previa
    ...
    <div className="min-h-screen flex items-center justify-center py-12 px-4">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <p className="text-center text-sm mt-4 text-gray-600">
          ¿No tienes cuenta? <a href="/register" className="text-primary-500 hover:text-primary-700 font-semibold">Regístrate</a>
        </p>
      </div>
    </div>
  );
